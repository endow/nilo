...
完了報告、レビュー用 Markdown、CLI 出力は入力または再生成可能な表示であり、DB より優先しない。

`.nilo/nilo.db` は稼働中の SQLite DB なので、DB 本体や `.db-wal` / `.db-shm` をクラウド同期ディレクトリへ直接置く運用は推奨しない。外部退避が必要な場合は、Nilo が SQLite backup API で作成した整合バックアップ成果物を使う。

バックアップ成果物は、単一の backup DB または暗号化ファイルと、隣接する `.meta.json` を一次記録にする。meta には reason、作成時刻、source、Nilo version、git 情報、sha256、integrity check、export / post command 結果など、DB を失った後でも由来と検証状態を確認するための情報を残す。バックアップ情報を `.nilo/nilo.db` 内だけに保存しない。

restore は destructive operation であり、通常の AI workflow が自律的に実行してよい操作ではない。人間の明示依頼がある場合だけ扱い、復元前には現在 DB を `before-restore` として退避し、復元対象と復元後 DB の integrity / sha256 を検証する。

世代管理は保守的に扱う。prune は明示的な keep 指定を必要とし、`manual`、`before-upgrade`、`before-migration`、`before-restore` のような安全網になる backup reason は既定で削除対象にしない。

外部連携は cloud API 認証を Nilo 本体に持ち込まない。必要な場合は export や argv 配列の post command で外部ツールへ引き渡す。post command は shell 文字列ではなく、制限された token 置換だけを行い、実行結果を backup meta に記録する。
...
自然文でユーザーが「タスク化して」「Task にして」「作業タスクを作って」と言った場合は、受付 `Todo` ではなく実行単位の `Task` 作成を意味する。`Todo` は、実行しない受付・候補・保留・後日の triage が明示された場合に使う。

### 人間向け翻訳層

Nilo の内部状態名は正確な監査語彙として保持するが、CLI / MCP / AI context の通常表示では、そのままユーザー判断の主語にしない。`Todo` は「受付済みの依頼」、`Task` は「実行する具体作業」、Roadmap は「実行許可前の作業計画」として説明する。

Roadmap は Nilo 内部では実行許可前の計画単位だが、人間向けには「作業計画」として表示する。承認待ち状態では、内部状態名ではなく、なぜ計画が必要か、何を確認すればよいか、承認後に何が起こるかを先に表示する。具体的には「作業が大きいため先に作業計画を作った」「この内容で進めてよいか確認する」「承認後に Task 化する」「修正したい場合は変更箇所を指示する」を示し、内部 ID や revision / commitment などの語は補足または machine-readable diagnostics に留める。
...
- `quick`: 小さな変更や局所的な smoke check。
- `targeted`: 変更した module、CLI group、振る舞いに対応する focused test。
- `full`: release や広範囲・高リスク変更で使う全体 test suite。

timeout は選択した検証範囲の実行 guardrail であり、全 task で full suite を既定にするための主手段ではない。
