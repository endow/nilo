...
| `tasks` | `src/nilo/transitions.py`, `src/nilo/work_service.py`, task creation handlers, roadmap/task planning helpers, MCP create endpoint, `src/nilo/workflow_services.py` base commit update | Yes for todo conversion and `work_service`; create/update exceptions documented | `work_service` is the projection-driven application boundary for daily Task creation. Instruction-time metadata is routed through `workflow_services`; other creation paths remain documented compatibility exceptions. |
| `instructions` | `src/nilo/workflow_services.py`, `src/nilo/work_service.py` | Yes in `work_service` | Instruction generation is append-only task workflow output. CLI adaptation delegates instruction creation to `workflow_services`; `work_service` checks for an existing Instruction before inserting one. |
| `understanding_checks` | `src/nilo/transitions.py`, `src/nilo/workflow_services.py` prepare/import | Yes for approval | Approval is transition-owned; prepare/import are append-only application-service inputs. |
