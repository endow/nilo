...
| `tasks` | `src/nilo/transitions.py`, `src/nilo/work_service.py`, task creation handlers, roadmap/task planning helpers, MCP create endpoint, `src/nilo/cli_handlers/workflow.py` base commit update | Yes for todo conversion and `work_service`; create/update exceptions documented | `work_service` is the projection-driven application boundary for daily Task creation. Other creation and instruction-time metadata paths remain compatibility exceptions until write routing is fully split. |
| `instructions` | `src/nilo/cli_handlers/workflow.py`, `src/nilo/work_service.py` | Yes in `work_service` | Instruction generation is append-only task workflow output. `work_service` checks for an existing Instruction before inserting one. |
