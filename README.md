...
Nilo は現在、個人開発での AI 開発フローを安定させるための実験的なツールです。API、DB スキーマ、CLI 出力は今後変更される可能性があります。本番システムの安全性、サンドボックス、認証、CI の代替ではありません。
...
完了、差し戻し、commit、作業方針の確定は人間の判断点です。AI が勝手に確定する前提にはしていません。
...
Nilo には、いま進める作業とは別に、後で扱う依頼や、複数作業にまたがる方針を残す仕組みがあります。

会話中に出た気づき、後続作業、今すぐやらない改善案は、すぐ実装せずに保留できます。方向性の確認が必要なものは、先に人間が方針を承認してから作業に分けます。
