...
Nilo は、AI に開発作業を任せるときの **現在地・完了条件・検証証跡・レビュー結果** を、プロジェクト内の `.nilo/` に残すための CLI ツールです。

AI の作業速度は活かしつつ、完了判断は人間が証跡を見て行えるようにします。
...
Nilo はこの曖昧さを `.nilo/nilo.db` に記録します。AI の自己申告だけに依存せず、作業状態をあとから確認できるようにします。
