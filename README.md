...
Nilo は現在、個人開発での AI 開発フローを安定させるための実験的なツールです。API、DB スキーマ、CLI 出力は今後変更される可能性があります。本番システムの安全性、サンドボックス、認証、CI、人間による最終判断の代替ではありません。Nilo は security boundary ではなく、証跡、監査、作業規律を記録するための道具です。
...
Nilo には、受け入れ済みの roadmap commitment に沿って、AI エージェントの作業を連続して進めるための **オーバードライブモード** があります。
...
特定の roadmap commitment を指定する場合は、次のように実行します。
...
- 予期しない dirty working tree (`unexpected_dirty_working_tree`)

また、最終的な human review checkpoint は必要です。

つまり、オーバードライブモードは AI に好き勝手進めさせるためのものではありません。Nilo が証跡、検証結果、レビュー結果、未解決事項を残しながら、人間の細かい承認待ちで作業を止めすぎないための運用モードです。AI の自己申告だけでは完了にせず、人間が最終的に受け入れ判断できる状態を残す、という Nilo の考え方は変わりません。Nilo は security boundary ではなく、evidence、audit、workflow discipline のためのツールです。
