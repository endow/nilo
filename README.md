...
`nilo view` は、Nilo に残ったタスク、検証、レビュー、失敗ログ、タスク分析をブラウザで確認するための読み取り専用ローカルビューです。既定では `127.0.0.1:8765` にだけ公開され、DB への書き込みは行いません。

## recipe / roadmap / overdrive

Recipe は、よくある作業を同じやり方で始めるための仕組みです。ドキュメント更新、不具合修正、リリース準備のような定型作業を、完了条件や検証観点つきで通常の Nilo Task にできます。
