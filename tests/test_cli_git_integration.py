    ...
    def test_ai_context_surfaces_are_compact_and_json_serializable(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIn("only when the user explicitly requests it", help_body)
                self.assertIn("nilo review claude --task <task_id> --user-requested", help_body)
                self.assertIn("MCP is not the normal entrypoint", help_body)
                self.assertIn("legacy/advanced/fallback paths", help_body)
                ...
                self.assertIn("mcp_default_tool_count: 15", doctor_body)
...
""",
            ...
            self.assertIn("review dirty-tree verification metadata", summary["next_actions"][0])
            ...
            self.assertIn("review dirty-tree verification metadata", text_summary_body)
            ...
            self.assertIn("review dirty-tree verification metadata", status_body)
