    ...
    def latest_task_status_event(self, task_id: str) -> dict[str, Any] | None:
        row = self.conn.execute(
            ...
            SELECT event_id, source, status, created_at FROM (
              ...
              UNION ALL
              SELECT id AS event_id, 'cancellation' AS source, 'cancelled' AS status, created_at, rowid AS event_rowid, 75 AS priority FROM outcome_reviews WHERE task_id=? AND decision='cancelled'
            )
            ORDER BY CASE WHEN source IN ('completion', 'cancellation') THEN 1 ELSE 0 END DESC, created_at DESC, priority DESC, event_rowid DESC
            LIMIT 1
            """,
